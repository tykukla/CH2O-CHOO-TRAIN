# --------------------------------------------------- # 
# Script to parameterize the beltland planet          # 
# so that total global land area is similar           #
# to the modern                                       #
# ---                                                 #
...
for(land.line in 1:length(lowest.lat)){
  ...
  land_alb_frac <- 1
  minlat <- lowest.lat[land.line] 
  newland <- modland # overwrite modland one at a time
  theserows <- newland[Lat >= minlat & Lat <= (minlat+belt.width), which = TRUE] # which statement returns row numbers: https://stackoverflow.com/questions/22408306/using-i-to-return-row-numbers-with-data-table-package
  # add land area
  newland[Lat >= minlat & Lat <= (minlat+belt.width)]$Land_Area_m2 <- land_alb_frac * lat.area[theserows]
  newland[Lat < minlat | Lat > (minlat+belt.width)]$Land_Area_m2 <- 0
  # add fraction of land in lat bin
  newland[Lat >= minlat & Lat <= (minlat+belt.width)]$LandFrac_inLatBelt_forAlbedo <- land_alb_frac
  newland[Lat < minlat | Lat > (minlat+belt.width)]$LandFrac_inLatBelt_forAlbedo <- 0
...
# ********************************************************* # 
# SAVE RESULT --------------------------------------------- #
saveRDS(out.list, file='Beltland_geography_2deg-LIST.RDS')  #
# ********************************************************* #
